...
robust_type_choice = "mean" # default of function
# robust_type_choice = "trimmed"
max_time = 80
df = NA # this hasn't been an issue in the past but since df is density of f distribution
# now I'm getting "object of type 'closure' is not subsettable" when it's used in a custom function
```
...
all.data <- all.data[all.data$Time <= max_time, ]
...
if(nrow(flag_after >= 1)){
  for(i in seq(1, nrow(flag_after))){
   mask <- (temp_2$Condition == unlist(flag_after[i, "Condition" ])
       ...
       ) & (temp_2$Time >= unlist(flag_after[i, "Time"])             #  < --------------test for djs
            )
...
## Full model with two conditional parameters
mk_asymptotic_model_2 <- function(
  df = temp_2,
  apply_qc = TRUE,
  robust_type = "mean"
){
...
write_out_fm_res <- function(fm = fm, 
  ...
  main_effect <- res[[1]]
  compare_d <- res[[2]]
  compare_d <- data.frame(compare_d)
  compare_d$holm <- p.adjust(compare_d[, 'p.value'], method = 'holm')
...
write_out_fm_summary <- function(fm = fm,
                                 save_suffix = ""){
  sink(paste0("./a_reg_summary_", save_suffix, ".txt"))
  print(summary(fm))
  sink()
}
...
delta_df_publication_plot <- function(
  ...
  plt <- df %>% 
    ...
    lemon::geom_pointline(aes(y=gj, group = interaction(Experiment, Inj_Cell)),
                          ...
                          linesize = line.size, alpha = 0.5, shape =1)+
    geom_point(aes(y=gj, group = interaction(Experiment, Inj_Cell)),
...
df = temp_2
df[df$Condition == "PS.90.HA.Cd", "Condition"] = "PS90HACd"
df[df$Condition == "PS.90.HA", "Condition"] = "PS90HA"
print("ANCOVA ON DELTAS ----- ")
fm <- lm(gj ~ Time*Condition, df[!df$remove,])
car::Anova(fm)
print(agricolae::HSD.test(fm, trt = c("Condition"))$groups)

print("ANOVA ON DELTAS ----- ")
fm <- lm(gj ~ Condition, df[(!df$remove)&(df$Time == 60),])
car::Anova(fm)
print(agricolae::HSD.test(fm, trt = c("Condition"))$groups)

print("KW ON DELTAS ----- ")
kruskal.test(gj ~ Condition, df[(!df$remove)&(df$Time == 60),] )
dunn.test::dunn.test(
  unlist(df[(!df$remove)&(df$Time == 60), "gj"]),
  unlist(df[(!df$remove)&(df$Time == 60), "Condition"]),
  method = "holm")
df = temp_2
...
fm <- mk_asymptotic_model(
  df = temp_2,
  apply_qc = TRUE,
  robust_type = robust_type_choice
)
...
## TEA+ha (TEST 4/16/2022)
```{r}
# ## What if we present all the high amp related treatmets together?
# 
# 
# savename = "gj_TEA_and_Depol_v1"
# 
# temp_2 <- mk_annotated_delta_df(
#   all.data = all.data,
#   threshold_change = 100.07,     # 
...
savename = "cc_ElevatedAmplitude"
threshold_change = 100.07    # 
threshold_change_res = 0.236 # <- max groupwise collapsed iqr
ith_condition = c("PS.0.High.Amp",  "PS.22.High.Amp", "PS.90.HA", "PS.90.HA.Cd")
...
savename = "r11_ElevatedAmplitude"
threshold_change = 100.07    # 

...
all.data <- all.data[all.data$Time <= 60, ]
...
### Note <------ Continue
```{r}
# Added custom fit for djs
savename <- "gj_HighAmpCd_Custom"
temp_2 <-  temp_2[temp_2$Condition== "PS.90.HA", ]

predictions <- mk_fm_plotting_df(df = temp_2, fm = fm)
predictions %>% head

backup_plt <- backup_plt+geom_line(data = predictions, aes(x = Time, y = Prediction), 
...
## Here's a heuristic complement to 
purrr::map(unique(temp_2$Condition), function(ith_c){
  mod <- summary(lm(gj ~ Time, temp_2[temp_2$Condition == ith_c, ]))
  print(ith_c)
  return(mod$coefficients[, 1])
  })
# [1] "PS.0.High.Amp"
#  (Intercept)         Time 
# -0.246977196  0.000520327 

# [1] "PS.22.High.Amp"
# (Intercept)        Time 
# -0.33295567 -0.01412646 

# [1] "PS.90.HA"
# (Intercept)        Time 
# -0.36792749 -0.01454095 

# [1] "PS.90.HA.Cd"
#  (Intercept)         Time 
# -0.112643995 -0.001528569 

## try with split time constant --
df = temp_2
apply_qc = TRUE
df <- df %>% mutate(Condition_tau = case_when(
  Condition == "PS.0.High.Amp" ~  "b",
  Condition == "PS.22.High.Amp" ~ "a",
  Condition == "PS.90.HA" ~       "a",
  Condition == "PS.90.HA.Cd" ~    "b"
  # abba pass
  # abbc fail
  # abca
  )

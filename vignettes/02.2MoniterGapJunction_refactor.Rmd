...
temp_2 <- mk_annotated_delta_df(
  all.data = all.data,
  threshold_change = 1.07,     # 
  threshold_change_res = 1.16, # <- max groupwise collapsed iqr
  # TODO   could this ^ be applied groupwise? That might fix this issue
  ith_condition = c("PS.0", "PS.22", "PS.45", "PS.90")
)
...
#TODO recalculate and add in summary bands.
plt2 <- delta_df_prediction_plot(
  df = temp_2,
  apply_qc = TRUE,
  predictions = predictions)

plt1+theme_linedraw() + plt2+theme_linedraw() 
```

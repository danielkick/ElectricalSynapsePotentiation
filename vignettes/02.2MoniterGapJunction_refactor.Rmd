...
# just like mk_annotated_delta_df but it automatically sets an IQR threshold based on each treatment not experiment wide

# mk_annotated_delta_df__groupwise_iqr
mk_annotated_delta_df <- function(
all.data = all.data,
# what is the largest change in 20 minutes that we're willing to accept as real?
# to be useful this might be something on the order of 1.5 but can be set high
...
temp_2[temp_2$absdiffgj > threshold_change, "remove"] = TRUE
# and add in residual
temp_2[temp_2$absresidgj > temp_2$max_iqr, "remove"] = TRUE # < ------------------------------------------
...
# temp2 <- mk_annotated_delta_df__groupwise_iqr(
#   all.data = all.data,
#   threshold_change = 100.07,     #
#   ith_condition = c("PS.0.High.Amp",  "PS.22.High.Amp", "PS.90.HA", "PS.90.HA.Cd")
# )
...
#   all.data = all.data,
#   threshold_change = 100.07,     #
#   threshold_change_res = 0.928, # <- max groupwise collapsed iqr
#   ith_condition = c("PS.0.High.Amp",  "PS.22.High.Amp", "PS.90.HA", "PS.90.HA.Cd")
# )
...
fm <- lm(gj ~ Time:Condition,
         data = df[!df$remove,])
car::Anova(fm)
print(agricolae::HSD.test(fm, trt = c('Condition')))

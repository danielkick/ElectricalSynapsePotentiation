...
temp = prepped.df[4][[1]]
t0 = temp[temp$Time == 0, ] %>% rename(t0 = dept.var)
t0 = t0 %>% dplyr::select(-Time)
temp = full_join(temp, t0)
temp[, 'dept.var'] = temp[, 'dept.var'] - temp[, 't0']
temp[, 'Experiment'] = paste0(temp[['Experiment']], temp[['Inj_Cell']])


ggplot(temp, aes(x = Time, y = dept.var, group = Experiment))+
  geom_point()+
  geom_line()+
...
target_groups == 'PS.0.High.Amp_PS.22.High.Amp'
target_dv = 'gj'

all.tests.list[[1]]$DV == 'gj'

for( i in seq_along(all.tests.list)){
  if(all.tests.list[[i]]$DV == 'gj'){
    print(i)
    print(unique(all.tests.list[[i]]$Groups))
  }
}

# [1] 1
# [1] PS.0.High.Amp_PS.22.High.Amp

# [1] 2
# [1] PS.22.High.Amp_PS.90.HA

# [1] 3
# [1] PS.90.HA_PS.90.HA.Cd

# [1] 4
# [1] PS.0_PS.22_PS.45_PS.90

i = 1
for(i in 1:4){
  temp = prepped.df[[i]]

  for(condition in unique(temp$Condition)){
    print(condition)
    tempSlice = temp[temp$Condition == condition & temp$Time %in% c(0, 40), ] %>%
      pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
      group_by(Experiment) %>%
      summarise(`0` = mean(`0`, na.rm =T),
                `40` = mean(`40`, na.rm =T))

    fm <- wilcox.test(tempSlice$`0`, tempSlice$`40`, paired = F)
    print(fm)
    # fm$p.value
    # fm$statistic
  }

  tempSlice = temp[temp$Time %in% c(40), ] %>%
      pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
      group_by(Experiment) %>%
      summarise(Condition = Condition,
                `40` = mean(`40`, na.rm =T))

  print(kruskal.test(`40` ~ Condition, tempSlice))
  print('--------------------------------------------------------------------------------')
}
...
for( i in seq_along(all.tests.list)){
  if(all.tests.list[[i]]$DV %in% c('gj', 'cc')){
    print(i)
    print(unique(all.tests.list[[i]]$Groups))
  }
}
...
for(i in c(1,2,3,4, 15,16,17,18,19)){
print(all.tests.list[[i]]$DV )
...
for(condition in unique(temp$Condition)){
  ...
  print('raw')
  fm <- t.test(tempSlice$`0`, tempSlice$`40`, paired = T)
  # fm <- wilcox.test(tempSlice$`0`, tempSlice$`40`, paired = T)#paired = F)
  print(fm$method)
  print(fm$statistic)
  print(fm$p.value)

  # tempSliceDelta = tempSlice['40'] - tempSlice['0']
  # fm2 <- wilcox.test(tempSliceDelta$`40`)
  # print('delta')
  # print(fm2$method)
  # print(fm2$statistic)
  # print(fm2$p.value)
...
#               `40` = mean(`40`, na.rm =T))
#
# fm3 <- kruskal.test(`40` ~ Condition, tempSlice)
# print('raw')
# print(fm3$method)
# print(fm3$parameter)
# print(fm3$statistic)
...
#
#
# tempSlice = temp[temp$Time %in% c(0, 40), ] %>%
#     pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
#     group_by(Experiment) %>%
#     summarise(Condition = Condition,
#               `0` = mean(`0`, na.rm =T),
#               `40` = mean(`40`, na.rm =T))
# tempSliceDelta = tempSlice
# tempSliceDelta['40'] = tempSliceDelta['40'] - tempSlice['0']
#
# fm4 <- kruskal.test(`40` ~ Condition, tempSliceDelta)
#
# print('delta')
# print(fm4$method)
# print(fm4$parameter)
...
Run non-parametrics on detlas
* all non 0 time points
* treat each observation as independent
* gj and cc
```{r eval=FALSE, include=FALSE}
...
outputList <- map(c(1,2,3,4, 15,16,17,18,19), function(i){
  df_for_deltas <- ready_data_to_test(input.df = M.clean,
                     dept.var = as.character(
                       all.tests.list[[i]][, "DV"]
                       # test.info[, "DV"]
                       ),
                     vis.conditions = str_split(string = as.character(
                       all.tests.list[[i]][, "Groups"]
                       # test.info[, "Groups"]
                       ),pattern = "_")[[1]], # Split up char string.
                     use.times = c(0, 20, 40, 60),
                     use.pseudorep.diff.tol = T,
                     use.diff.tol = 0.5,
                     obs.is.network = F
                       # as.logical(test.info[, "Obs.Net"])
                     )

  dv = all.tests.list[[i]][, "DV"]

  conditions <- unique(df_for_deltas$Condition)

  # condition <- list(conditions)[[1]][1]

  list_of_outDfs <- map(conditions, function(condition){
    outDf = data.frame(
      'Dv' = dv,
      'Condition' = condition,
      'Time' = rep(c(20, 40, 60), each  = 3),
      'Key' = rep(c('method', 'v', 'p'), times = 3))

    df_for_deltas_20 <- df_for_deltas[df_for_deltas$Time %in% c(0, 20), ] %>%
      pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
      mutate(diff = `20`-`0`)
    fm20 <- wilcox.test(df_for_deltas_20[df_for_deltas_20$Condition == condition, ]$diff)

    df_for_deltas_40 <- df_for_deltas[df_for_deltas$Time %in% c(0, 40), ] %>%
      pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
      mutate(diff = `40`-`0`)
    fm40 <- wilcox.test(df_for_deltas_40[df_for_deltas_40$Condition == condition, ]$diff)

    df_for_deltas_60 <- df_for_deltas[df_for_deltas$Time %in% c(0, 60), ] %>%
      pivot_wider(names_from = 'Time', values_from = 'dept.var') %>%
      mutate(diff = `60`-`0`)
    fm60 <- wilcox.test(df_for_deltas_60[df_for_deltas_60$Condition == condition, ]$diff)
    ...
    outDf['stars'] = ''
    outDf[outDf$Key == 'p' & outDf$value < 0.1, 'stars'] = '.'
    outDf[outDf$Key == 'p' & outDf$value < 0.05, 'stars'] = '*'
    outDf[outDf$Key == 'p' & outDf$value < 0.01, 'stars'] = '***'

    return(outDf)
  })

  bound_outDfs <- do.call(rbind, list_of_outDfs)
  bound_outDfs['i'] = i
  return(bound_outDfs)
})

desiredTestRes = do.call(rbind, outputList)

write.csv(desiredTestRes, file = 'C:/Users/Daniel/Desktop/nonParametric_Delta_obs_as_cell.csv')
```
...
# try pairwise test for reviewer 2: 2020 12 18
```{r eval=FALSE, include=FALSE}
str(M.clean)

## "PS.0.High.Amp", "PS.22.High.Amp" ============================================================================
temp <- M.clean[M.clean$Condition %in% c("PS.0.High.Amp", "PS.22.High.Amp"), ] %>%
  dplyr::select(Condition, Experiment, Inj_Cell, Time, gj) %>%
  group_by(Condition, Experiment, Inj_Cell, Time) %>%
  summarise(gj = mean(gj, na.rm = T)) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  group_by(Condition, Experiment, Time) %>%
  mutate(gj = mean(gj, na.rm = T)) %>%
  filter(Time == 40) %>%
  dplyr::select(-"Inj_Cell") %>%
  distinct() %>%
  filter(!is.na(gj)) %>%
  ungroup()

temp %>%
  ggplot(aes(Condition, gj))+
  geom_point()


# ts <- map_df(1:1000, function(i){
#   t.test(gj ~ sample(temp$Condition), temp)$statistic
# })$t

print("tea mimic")
# mean(abs(ts)>=t.test(gj ~ Condition, temp)$statistic)
# t.test(gj ~ Condition, temp)$p.value
wilcox.test(gj ~ Condition, temp)$p.value
...
## "PS.90.HA", "PS.22.High.Amp" ============================================================================
temp <- M.clean[M.clean$Condition %in% c("PS.90.HA", "PS.22.High.Amp"), ] %>%
  dplyr::select(Condition, Experiment, Inj_Cell, Time, gj) %>%
  group_by(Condition, Experiment, Inj_Cell, Time) %>%
  summarise(gj = mean(gj, na.rm = T)) %>%
  ungroup() %>%
...
tost(temp[temp$Condition == "PS.22.High.Amp", ]$gj,
     temp[temp$Condition == "PS.90.HA", ]$gj,
     epsilon = 1, paired = F, var.equal = T, conf.level = 0.95)

wilcox.test(gj ~ Condition, temp)

## "PS.90.HA" ============================================================================
temp <- M.clean[M.clean$Condition %in% c("PS.90.HA"), ] %>%
  dplyr::select(Condition, Experiment, Inj_Cell, Time, gj) %>%
  group_by(Condition, Experiment, Inj_Cell, Time) %>%
  summarise(gj = mean(gj, na.rm = T)) %>%
  ungroup() %>%
...
temp %>%
  ggplot(aes(Time, gj, group = Experiment))+
  geom_line()+
  geom_point()

temp2 <- temp %>%  pivot_wider(names_from = Time, values_from = gj)
wilcox.test(temp2$`0`, temp2$`40`, paired = T)


## "PS.0", "PS.22", "PS.45", "PS.90" ============================================================================

temp <- M.clean[M.clean$Condition %in% c("PS.0", "PS.22", "PS.45", "PS.90"), ] %>%
  dplyr::select(Condition, Experiment, Inj_Cell, Time, gj) %>%
  group_by(Condition, Experiment, Inj_Cell, Time) %>%
  summarise(gj = mean(gj, na.rm = T)) %>%
  ungroup() %>%
...
# mean(abs(ts)>=t.test(gj ~ Condition, temp)$statistic)
# t.test(gj ~ Condition, temp)$p.value
kruskal.test(gj ~ Condition, temp)$p.value
...
temp %>%
  ggplot(aes(Time, gj, group = Condition))+
  geom_point()+
  geom_smooth()+
  facet_grid(.~Condition)

wilcox.test(temp[temp$Condition == "PS.0",]$gj)#$p.value
wilcox.test(temp[temp$Condition == "PS.22",]$gj)#$p.value
wilcox.test(temp[temp$Condition == "PS.45",]$gj)#$p.value
wilcox.test(temp[temp$Condition == "PS.90",]$gj)#$p.value
...
wilcox.test(gj ~ Time, temp[temp$Condition == "PS.0",])#$p.value
wilcox.test(gj ~ Time, temp[temp$Condition == "PS.22",])#$p.value
wilcox.test(gj ~ Time, temp[temp$Condition == "PS.45",])#$p.value
wilcox.test(gj ~ Time, temp[temp$Condition == "PS.90",])#$p.value
...
#
#
# temp <- M.clean[M.clean$Condition %in% c("PS.0.orig", "PS.22.orig", "PS.45.orig", "PS.90.orig", "PS.180.orig"), ] %>%
#
#   dplyr::select(-gj) %>% # make sure we're workign with the cc measures since this was when we first added the tevc protocols
#   rename(gj = rc) %>%
#
#   dplyr::select(Condition, Experiment, Inj_Cell, Time, gj) %>%
#   group_by(Condition, Experiment, Inj_Cell, Time) %>%
...
#   facet_grid(.~Condition)
#
# wilcox.test(temp[temp$Condition == "PS.0.orig",]$gj)$p.value
# wilcox.test(temp[temp$Condition == "PS.22.orig",]$gj)$p.value
# wilcox.test(temp[temp$Condition == "PS.45.orig",]$gj)$p.value
# wilcox.test(temp[temp$Condition == "PS.90.orig",]$gj)$p.value
# wilcox.test(temp[temp$Condition == "PS.180.orig",]$gj)$p.value
...
g1 <- c("PS.0.High.Amp", "PS.22.High.Amp")
g2 <- c("PS.90.HA", "PS.90.HA.Cd")
g3 <- c("PS.0", "PS.22", "PS.45", "PS.90")
g4 <- c("PS.0.orig", "PS.22.orig", "PS.45.orig", "PS.90.orig", "PS.180.orig")


# tea ----
temp[temp$Condition %in% g1, ] %>%
  ggplot(aes(Time, gj, group = Experiment))+
...
kruskal.test(gj ~ Condition, temp[temp$Condition %in% g1 & temp$Time == 40, ])$p.value


# ha ----
temp[temp$Condition %in% g2, ] %>%
  ggplot(aes(Time, gj, group = Experiment))+
  geom_hline(yintercept = 0, color="red", linetype = "dashed")+
  geom_line()+
  geom_point()+
  geom_line(data = temp[temp$Condition %in% g2 & temp$Experiment == "191204", ], color = "#7a7a7a")+
  geom_point(data = temp[temp$Condition %in% g2 & temp$Experiment == "191204", ], color = "#7a7a7a")+
  facet_grid(.~Condition)

kruskal.test(gj ~ Condition, temp[temp$Condition %in% g2 & temp$Time == 40, ])$p.value
# esclude that one weird experiment
kruskal.test(gj ~ Condition, temp[temp$Condition %in% g2 & temp$Time == 40 & temp$Experiment != "191204", ])$p.value


# standard ----
temp[temp$Condition %in% g3, ] %>%
  ggplot(aes(Time, gj, group = Experiment))+
  geom_hline(yintercept = 0, color="red", linetype = "dashed")+
  geom_line()+
  geom_point()+
...
kruskal.test(gj ~ Condition, temp[temp$Condition %in% g3 & temp$Time == 40, ])$p.value

# Is 90 really different?
for (i in 1:3){
  kruskal.test(gj ~ Condition, temp[temp$Condition %in% g3[c(i,4)] & temp$Time == 40, ])$p.value %>% print()
}
...
#   "PS.0.orig", "PS.22.orig", "PS.45.orig", "PS.90.orig", "PS.180.orig"
#   ), ] %>%
#   dplyr::select(Condition, Experiment, Inj_Cell, Time, rc) %>%
#   group_by(Condition, Experiment, Inj_Cell, Time) %>%
#   summarise(rc = mean(rc, na.rm = T)) %>%
#   ungroup() %>%
#   # mutate(Time = as.character(Time)) %>%
...
#
#
# temp[temp$Condition %in% g4, ] %>%
#   ggplot(aes(Time, rc, group = Experiment))+
#   geom_hline(yintercept = 0, color="red", linetype = "dashed")+
#   geom_line()+
#   geom_point()+
#   facet_grid(.~Condition)
#
# kruskal.test(rc ~ Condition, temp[temp$Condition %in% g4 & temp$Time == 40, ])$p.value
